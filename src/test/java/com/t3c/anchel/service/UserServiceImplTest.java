...
import com.t3c.anchel.core.domain.constants.AccountType;
import com.t3c.anchel.core.domain.constants.FunctionalityNames;
...
import com.t3c.anchel.core.domain.entities.AllowedContact;
import com.t3c.anchel.core.domain.entities.DenyAllDomain;
...
import com.t3c.anchel.core.repository.AllowedContactRepository;
import com.t3c.anchel.core.repository.DomainAccessPolicyRepository;
...
public class UserServiceImplTest extends
	...
	public void testCreateGuest() throws BusinessException {
		...
		Internal user = new Internal("John", "Doe", "user1@t3c.io", null);
		...
		try {
			...
			Guest guest = new Guest("Guest", "Doe", "guest1@t3c.io");
		...
		Assert.assertNotNull(userRepository.findByMail("guest1@t3c.io"));
	...
	public void testFindUserInDB() {
		...
		Assert.assertNotNull(userService.findUserInDB(
				LoadingServiceTestDatas.sqlSubDomain, "user1@t3c.io"));
		logger.debug(LinShareTestConstants.END_TEST);
	}
	...
	public void testDeleteUser() throws BusinessException {
		...
		AbstractDomain subDomain = abstractDomainRepository
				.findById(LoadingServiceTestDatas.sqlSubDomain);
		User user1 = new Internal("John", "Doe", "user1@t3c.io", null);
		user1.setDomain(topDomain);
		user1.setRole(Role.ADMIN);
		User user2 = new Internal("Jane", "Smith", "user2@t3c.io", null);
		...
		try {
			...
			User u = userService.findUserInDB(
					LoadingServiceTestDatas.sqlSubDomain, "user2@t3c.io");
		...
		Assert.assertNull(userService.findUserInDB(
				LoadingServiceTestDatas.sqlSubDomain, "user2@t3c.io"));
		Assert.assertNotNull(userService.findUserInDB(
				LoadingServiceTestDatas.sqlDomain, "user1@t3c.io"));
		wiser.checkGeneratedMessages();
		logger.debug(LinShareTestConstants.END_TEST);
	}
	...
	public void testDeleteAllUsersFromDomain() throws BusinessException {
		...
		User user2 = new Internal("Jane", "Smith", "user2@t3c.io", null);
		user2.setDomain(subDomain);
		User user3 = new Internal("Foo", "Bar", "user3@t3c.io", null);
		...
		Assert.assertNull(userService.findUserInDB(
				LoadingServiceTestDatas.sqlSubDomain, "user2@t3c.io"));
		Assert.assertNull(userService.findUserInDB(
				LoadingServiceTestDatas.sqlSubDomain, "user3@t3c.io"));
	...
	public void testIsAdminForThisUser() {
		...
		AbstractDomain subDomain = abstractDomainRepository
				.findById(LoadingServiceTestDatas.sqlSubDomain);
		Internal user1 = new Internal("John", "Doe", "user1@t3c.io", null);
		user1.setDomain(rootDomain);
		user1.setRole(Role.ADMIN);
		Internal user2 = new Internal("Jane", "Smith", "user2@t3c.io", null);
		user2.setDomain(subDomain);
		user1.setCmisLocale("en");
		user2.setCmisLocale("en");
	...
	public void testUpdateGuest() throws BusinessException {
		...
		// create guest
		Guest guest = new Guest("Foo", "Bar", "user3@t3c.io");
	...
	public void testChangePassword() throws BusinessException {
		...
		AbstractDomain topDomain = abstractDomainRepository
				.findById(LoadingServiceTestDatas.sqlDomain);
		Guest user1 = new Guest("John", "Doe", "user-unknow@t3c.io");
		user1.setDomain(topDomain);
		...
		userService.changePassword(user1.getLsUuid(), "user1@t3c.io",
...
//				HashUtils.hashSha1withBase64(oldPassword.getBytes())));
	...
	public void testAddGuestContactRestriction() throws BusinessException {
		...
		for (AllowedContact allowedContact : listAllowedContact) {
			if (allowedContact.getContact().getMail().equals("user1@t3c.io")) {
				test = true;
			}
		}
...
//
//		// create guest
//		Guest guest2 = new Guest("Jane", "Smith", "user2@t3c.io");
//		guest2.setDomain(abstractDomainRepository
//				.findById(LoadingServiceTestDatas.sqlGuestDomain));
//		guest2.setOwner(user1);
	...
	public void testFindOrCreateUser() {
		...
		try {
			...
			User user = userService.findOrCreateUserWithDomainPolicies(
					"user1@t3c.io", LoadingServiceTestDatas.sqlSubDomain);
		...
		try {
			...
			User user = userService.findOrCreateUser("user1@t3c.io",
					LoadingServiceTestDatas.sqlSubDomain);
			Assert.assertEquals("John", user.getFirstName());
			Assert.assertEquals("Doe", user.getLastName());
